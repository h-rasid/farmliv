...
const StaffManagementPage = () => {
  ...
  const handleAddStaff = async (e) => {
    ...
    try {
      ...
      toast({ title: "Account Initialized", description: `${formData.name} added as ${formData.role}` });
    ...
    <PortalLayout role="admin">
      <div className="max-w-[1600px] mx-auto p-10 space-y-12 font-sans text-slate-900">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          {/* ⭐ RECRUITMENT SECTION (Add Admin/Sales Executive) */}
          <div className="lg:col-span-1">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="sticky top-32 bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 space-y-8">
              <div className="space-y-1">
                <h3 className="text-lg font-semibold uppercase flex items-center gap-3"><UserPlus className="text-emerald-600" size={20}/> Onboard Executive</h3>
            ...
            {view === 'directory' ? (
              <div className="space-y-6">
                <div className="relative group">
                  ...
                  <input type="text" placeholder="Filter staff directory..." className="w-full pl-14 pr-6 py-5 bg-white border border-slate-100 rounded-[2rem] shadow-sm text-xs font-medium outline-none" onChange={(e) => setSearchTerm(e.target.value)} />
                </div>
              ...
              <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
                ...
                <div className="p-4 space-y-2">
                  {logs.length > 0 ? logs.map((log, idx) => (
                    ...
                    <div className="py-20 text-center text-slate-300 uppercase text-[10px] font-bold tracking-[0.2em]">No activities recorded recently</div>
                  )}
                </div>
              </div>
            )}
